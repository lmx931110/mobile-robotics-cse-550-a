...
//public:  
SensorScan::SensorScan(LaserProxy * newLaser, Position2dProxy * newPosition, MapWorldApi* newCurrentWorldView, double newFunctionChoice, double newTargetPath)
{
	laser = newLaser;
	position = newPosition;
	currentWorldView = newCurrentWorldView;
...
void SensorScan::ReadSensors(){
	Pose currentPose((*position).GetXPos(), (*position).GetYPos(), rtod((*position).GetYaw()));
	(*currentWorldView).ReadSensor(currentPose);
